...
def main():
    ...
    parser.add_argument('--keep_embeddings_in_memory',
                        help='keep embeddings in memory (useful for non reconstructible index)',
                        type=bool, default=False)
    parser.add_argument('--root_embeddings_dir', help='dir to save embeddings', type=str,
                        default=home + "/embeddings")
    parser.add_argument('--embeddings_id', help='word embeddings id from http://vectors.nlpl.eu/repository/',
                        type=str,
                        default="0")
    ...
    closest_words = from_nlpl(args.root_embeddings_dir, args.embeddings_id,
                              args.save_zip, args.keep_embeddings_in_memory)
    if not args.serve :
        ...
        class NearestWords(Resource):
            def get(self, word, count, with_distances):
                return closest_words.closest_words(word, count, with_distances == "distance",
                                                   args.keep_embeddings_in_memory)
